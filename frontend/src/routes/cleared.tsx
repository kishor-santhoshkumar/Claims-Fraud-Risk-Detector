...
function ClearedPage() {
  ...
  const cleared = providers.filter((p) => p.status === "cleared");
  ...
  const undo = (p: CaseProvider) => {
    ...
    toast(`${p.provider_id} moved back to unreviewed`, {
      action: { label: "Redo", onClick: () => setStatus(p.provider_id, "cleared") },
    });
  };

  const totalReimbursed = cleared.reduce((s, p) => s + p.total_reimbursed, 0);

  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100svh" }}>
      <header style={{ padding: "20px 24px 16px", backdropFilter: "blur(20px) saturate(180%)", WebkitBackdropFilter: "blur(20px) saturate(180%)", background: "linear-gradient(160deg, rgba(255,255,255,0.72) 0%, rgba(228,253,228,0.4) 100%)", borderBottom: "1px solid rgba(255,255,255,0.75)" }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, color: "var(--text-primary)", margin: 0 }}>Cleared Providers</h1>
        <p style={{ fontSize: 12, color: "var(--text-faint)", margin: "4px 0 0" }}>
          {cleared.length.toLocaleString()} providers cleared · {formatMoneyShort(totalReimbursed)} total reimbursed
        </p>
      </header>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 32px" }}>
        {cleared.length === 0 ? (
          <div style={{ paddingTop: 60, textAlign: "center" }}>
            <p style={{ fontSize: 14, color: "var(--text-muted)" }}>No providers cleared yet.</p>
            <p style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 6 }}>
              Clear providers from the <Link to="/queue" style={{ color: "#3b82f6" }}>review queue</Link> or individual case pages.
            </p>
          </div>
        ) : (
            ...
            {cleared.map((p, i) => (
              ...
              >
                ...
                <Link
                  ...
                  style={{ fontFamily: "ui-monospace,monospace", color: "#059669", textDecoration: "none", fontWeight: 600 }}
                >
                  {p.provider_id}
                </Link>
